...
public class PosGraphController
{
    ...
    private void setBGImg()
    {
        ...
        {
            File imgFile = new File( SettingsModel.getSettingsDir() + "/FRC2020.png" );

            if( !imgFile.exists() )
            {
                try
                {
                    ResourceLoader.resourceToFile("/images/FRC2020.png", imgFile );
                }
                catch( IOException e )
                {
                    return;
            ...
            settings.setGraphBGImagePath( SettingsModel.getSettingsDir() + "/FRC2020.png" );
        }
        else
        {
